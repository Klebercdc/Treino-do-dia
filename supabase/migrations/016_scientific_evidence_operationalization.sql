-- KRONIA — Migração 016: operacionalização corretiva do banco científico
...
ALTER TABLE public.scientific_articles
  ...
  ADD CONSTRAINT scientific_articles_evidence_score_chk CHECK (evidence_score IS NULL OR (evidence_score >= 0 AND evidence_score <= 100)),
  DROP CONSTRAINT IF EXISTS scientific_articles_confidence_label_chk,
  ADD CONSTRAINT scientific_articles_confidence_label_chk CHECK (
    confidence_label IS NULL OR confidence_label IN ('very_low', 'low', 'moderate', 'high', 'very_high')
  );
...
CREATE INDEX IF NOT EXISTS ix_scientific_articles_published_at
  ON public.scientific_articles (published_at DESC);
...
INSERT INTO public.scientific_topics (topic, keywords, status)
VALUES
  ('hypertrophy', ARRAY['muscle gain', 'resistance training', 'volume'], 'active'),
  ('strength', ARRAY['max strength', 'powerlifting', 'progressive overload'], 'active'),
  ('creatine', ARRAY['creatine monohydrate', 'supplementation', 'performance'], 'active'),
  ('protein', ARRAY['dietary protein', 'muscle protein synthesis', 'intake'], 'active'),
  ('fat loss', ARRAY['body fat', 'energy deficit', 'weight management'], 'active'),
  ('recovery', ARRAY['sleep', 'fatigue', 'muscle recovery'], 'active'),
  ('women training', ARRAY['female physiology', 'menstrual cycle', 'training adaptation'], 'active')
