...
function normalizeWorkoutPayload(payload) {
  ...
  return {
    objetivo: pickString(safePayload.objective, safePayload.objetivo, safePayload.goal, profile.objective, profile.objetivo, profile.goal, context.objective, context.goal),
    nivel: pickString(safePayload.level, safePayload.nivel, safePayload.experience, safePayload.experienceLevel, profile.level, profile.nivel, profile.experience, profile.experienceLevel, context.level, context.experience),
    dias: dias ? String(Math.max(1, Math.min(6, dias))) : undefined,
    tempo: pickString(safePayload.tempo, safePayload.duration, safePayload.sessionLength, profile.tempo, profile.duration, profile.sessionLength, context.tempo, context.duration, context.sessionLength),
    equipamentos: pickString(safePayload.environment, safePayload.equipamentos, safePayload.equipment, profile.environment, profile.equipamentos, profile.equipment, context.environment, context.equipamentos, context.equipment),
    limitacoes: pickString(
      safePayload.limitacoes,
      safePayload.limitations,
      safePayload.injuries,
      safePayload.notes,
