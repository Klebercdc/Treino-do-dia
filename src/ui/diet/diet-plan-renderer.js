...
(function(){
  ...
  var LAB_CONTEXT_KEY = 'kronia_latest_lab_context';

  function $(id){ return document.getElementById(id); }
  function esc(v){ return String(v == null ? '' : v).replace(/[&<>"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c];}); }
  function n(v){ return Math.round(Number(v)||0); }
  function readJson(key){ try { return JSON.parse(localStorage.getItem(key) || 'null'); } catch(_) { return null; } }
  function readPlan(){ return readJson(LAST_PLAN_KEY); }
  ...
  function getLabContext(plan){
    return (plan && (plan.labContext || (plan.context && plan.context.labContext) || (plan.profile && plan.profile.labContext) || plan.healthExamContext)) || readJson(LAB_CONTEXT_KEY);
  }
  ...
  function formatDate(v){
    ...
    try { return d.toLocaleDateString('pt-BR',{timeZone:'America/Sao_Paulo'}); } catch(_) { return d.toLocaleDateString('pt-BR'); }
  }

  function renderLabsContext(plan){
    var lab = getLabContext(plan);
    var biomarkers = lab && Array.isArray(lab.biomarkers) ? lab.biomarkers : [];
    if(!lab || !biomarkers.length){
      return '<div class="kdp-labs" data-diet-labs-open><div class="kdp-labs-top"><div class="kdp-labs-title">Biomarcadores reais</div><div class="kdp-labs-badge">Não conectado</div></div><div class="kdp-labs-sub">A dieta ainda não está exibindo marcadores reais. Toque abaixo para abrir o pipeline real de exames do KroniA.</div><button class="kdp-lab-btn" data-diet-labs-open type="button">Abrir exames reais</button></div>';
    }
    var chips = biomarkers.slice(0,8).map(function(b){
      var value = b.value == null || b.value === '' ? '—' : String(b.value).replace('.', ',');
      var unit = b.unit ? ' ' + b.unit : '';
      var flag = /high|alto/i.test(b.flag || '') ? ' ↑' : /low|baixo/i.test(b.flag || '') ? ' ↓' : '';
      return '<div class="kdp-lab-chip"><strong>'+esc(b.name || 'Marcador')+'</strong>'+esc(value + unit + flag)+'</div>';
    }).join('');
    var when = formatDate(lab.processedAt || lab.createdAt);
    var flags = [].concat(Array.isArray(lab.clinicalFlags) ? lab.clinicalFlags : [], Array.isArray(lab.criticalFlags) ? lab.criticalFlags : []).slice(0,2);
    return '<div class="kdp-labs" data-diet-labs-open><div class="kdp-labs-top"><div class="kdp-labs-title">Biomarcadores usados na dieta</div><div class="kdp-labs-badge">Conectado</div></div><div class="kdp-labs-sub">Exame real do KroniA'+(when?' · '+esc(when):'')+'. Esses dados devem orientar ajustes nutricionais e alertas.</div><div class="kdp-labs-grid">'+chips+'</div>'+(flags.length?'<div class="kdp-note">Alertas reais: '+flags.map(esc).join(' · ')+'</div>':'')+'<button class="kdp-lab-btn" data-diet-labs-open type="button">Ver exames completos</button></div>';
  }
  ...
  function renderDietFromPlan(plan){
    ...
    screen.innerHTML = '<div class="kdp-head"><div class="kdp-top"><button class="kdp-close" id="kdpClose">×</button><div style="flex:1;min-width:0"><div class="kdp-badge">Dieta gerada</div><div class="kdp-title">'+esc(plan.title || 'Plano alimentar KroniA')+'</div><div class="kdp-sub">Plano salvo e renderizado pelo KroniA, conectado ao contexto real de exames quando disponível.</div></div></div></div><div class="kdp-body"><div class="kdp-hero"><div class="kdp-sub">Meta diária</div><div class="kdp-kcal">'+n((plan.target&&plan.target.kcal) || (plan.totals&&plan.totals.kcal))+' <span>kcal</span></div><div class="kdp-macros"><div class="kdp-macro"><strong>'+n(plan.totals&&plan.totals.protein)+'g</strong><span>Proteína</span></div><div class="kdp-macro"><strong>'+n(plan.totals&&plan.totals.carbs)+'g</strong><span>Carbo</span></div><div class="kdp-macro"><strong>'+n(plan.totals&&plan.totals.fat)+'g</strong><span>Gordura</span></div></div></div>'+renderLabsContext(plan)+'<div class="kdp-section">Refeições de hoje</div>'+plan.meals.map(renderMeal).join('')+(warnings.length?'<div class="kdp-note">'+warnings.map(esc).join('<br><br>')+'</div>':'')+'</div><div class="kdp-foot"><button class="kdp-primary" id="kdpNewDiet">Gerar nova dieta</button><button class="kdp-secondary" id="kdpBackDiet">Voltar para Dieta</button></div>';
    ...
    var labsBtn = screen.querySelector('[data-diet-labs-open]');
    if (labsBtn) labsBtn.onclick = function(ev){ if(ev){ev.preventDefault();ev.stopPropagation();} if(window.KroniaDietLabs && typeof window.KroniaDietLabs.openRealLabs === 'function') window.KroniaDietLabs.openRealLabs('diet-plan-renderer-labs-card'); };
