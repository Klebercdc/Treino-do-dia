...
(function () {
  ...
  window.ffObFinish = function () {
    ...
    try { window.KroniaIntelligence?.track?.({ module: 'onboarding', action: 'onboarding_complete', status: 'start', correlationId: correlationId, source: 'fitflow_layout' }); } catch (_) {}
    ...
    if (completion.status === 'error') {
      try { window.KroniaIntelligence?.track?.({ module: 'onboarding', action: 'onboarding_complete', status: 'error', correlationId: correlationId, durationMs: Date.now() - startedAt, source: 'fitflow_layout' }); } catch (_) {}
      if (typeof showToast === 'function') showToast('Erro ao concluir onboarding.', 'error');
      return;
    }
    try { window.KroniaIntelligence?.track?.({ module: 'onboarding', action: 'onboarding_complete', status: 'success', correlationId: correlationId, durationMs: Date.now() - startedAt, source: 'fitflow_layout' }); } catch (_) {}
