...
function initTrial() {
  if (!localStorage.getItem('kronia_trial_start')) {
    localStorage.setItem('kronia_trial_start', Date.now().toString());
  }
}

function getTrialStatus() {
  var start = localStorage.getItem('kronia_trial_start');
  if (!start) return null;
  var startMs  = parseInt(start);
  var nowMs    = Date.now();
  var diffMs   = nowMs - startMs;
...
async function fetchUserPlan() {
  try {
    ...
    if (res.data) {
      _userPlan = {
        plan: res.data.plan,
        ai_requests_used: res.data.ai_requests_used || 0,
        limit: FREE_AI_LIMIT
      };
      updatePlanBadge();
    }
...
function updatePlanBadge() {
  ...
  var rem     = Math.max(0, (inTrial ? TRIAL_AI_LIMIT : FREE_AI_LIMIT) - _userPlan.ai_requests_used);
